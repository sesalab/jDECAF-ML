 ...
 * @author fabiano
 */
public class BalancingComparison {

    public BalancingComparison(String predictorsFilePath, String outputFilePath, String overlapFilePath, Classifier classifier, int rep, List<ClassBean> classes, List<MethodBean> methods, boolean classSmell, DetectionRule dr) {
    ...
    private static void evaluateModel(String outputFilePath, String overlapFilePath, Classifier pClassifier, Instances pInstances, int rep, List<ClassBean> classes, List<MethodBean> methods, boolean classSmell, DetectionRule detector) throws FileNotFoundException, Exception {
        ...
        String output = "r;TP;FP;FN;TN;Accuracy;Precision;Recall;fmeasure;areaUnderROC;MCC;index;time\n";
...
//                    }
                    ...
                    if (r == 0);
                           ...
                           Filter dataBalancingFilter = createBalancingFilter2(trainingSet);
                           trainingSet = Filter.useFilter(trainingSet, dataBalancingFilter);                        
                    }
                    else if (r == 3) {
                           Filter dataBalancingFilter = createBalancingFilter3(trainingSet);
                           trainingSet = Filter.useFilter(trainingSet, dataBalancingFilter);                        
                    }
                    else if (r == 4){
                        int weight = (int) Math.round(total/smelly);
                        Classifier old = pClassifier;
                        pClassifier = new CostSensitiveClassifier();
                        
                        ((CostSensitiveClassifier)pClassifier).setClassifier(old);
                    }
                    else if (r == 5){
                        Classifier old = pClassifier;
                        pClassifier = new OneClassClassifier();
                        ((OneClassClassifier)pClassifier).setTargetClassLabel("true");
                    }
                    //   Filter dataBalancingFilter = createBalancingFilter(trainingSet);
                    //  trainingSet = Filter.useFilter(trainingSet, dataBalancingFilter);
...
//                    }
                    ...
                    if (r==4){
                        Evaluation maxEval;
                        double maxMCC;
                        ((CostSensitiveClassifier)pClassifier).setCostMatrix(CostMatrix.parseMatlab("[0 1; 2 0]"));
                        pClassifier.buildClassifier(trainingSet);
                        eval.evaluateModel(pClassifier, testSet);
                        maxMCC = eval.matthewsCorrelationCoefficient(positiveValueIndexOfClassFeature);
            ...
            output += r+ ";" + tp + ";" + fp + ";" + fn + ";" + tn + ";" + accuracy + ";" + precision + ";" + recall + ";" + fmeasure + ";" + auc + ";" + MCC + ";" + maxIndex + ";" + enlapsedTime + "\n";
            failed = false;
